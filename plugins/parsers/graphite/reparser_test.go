...
var (
	re_templates []string = []string{
		...
		"(?:^MfmsMonitor)\\.(?P<component>[\\w-]+?\\d+)\\.(?P<zone>\\w+?)\\.(?P<measurement>.+?payloadOutPacketQueue)\\.(?P<peer>[\\w]+?)\\.(?P<measurement>.+$)",
		"(?:^MfmsMonitor)\\.(?P<component>[\\w-]+?\\d+)\\.(?P<zone>\\w+?)\\.(?P<measurement>.+?channelInMessageProcessMonitorAvgThroughputCounter)\\.(?P<subject>.+$)",
		"(?:^MfmsMonitor)\\.(?P<component>[\\w-]+?\\d+)\\.(?P<zone>\\w+?)\\.(?P<measurement>.+?PercentileCounter)\\.(?P<percentile>\\w+$)",
...
func TestTemplateApplyReParser(t *testing.T) {
	var tests = []struct {
		...
		},
		{
			...
			tags:        map[string]string{"component": "connector-hpx-rbr1", "zone": "zcnr06", "peer": "kazakhstan-tele2", "period": "3600"},
		},
		{
			...
			tags:        map[string]string{"component": "connector-hpx-rbr1", "zone": "zcnr06", "peer": "russia-ktktelecom", "status": "PROVIDER_UNDELIVERED"},
		},
		{
			input:       "MfmsMonitor.connector-hpx-rbr1.zcnr06.n1.delivery.source.e-staff.period.43200",
			measurement: "n1.delivery.source.period",
			tags:        map[string]string{"component": "connector-hpx-rbr1", "zone": "zcnr06", "peer": "e-staff", "period": "43200"},
		},
		{
			...
			tags:        map[string]string{"component": "connector-hpx-rbr1", "zone": "zcnr06", "peer": "stepUP", "status": "PROVIDER_FAILED"},
		},
		{
			...
			tags:        map[string]string{"component": "connector-hpx-rbr1", "zone": "zcnr06"},
		},
		{
		...
		},
		{
			...
			tags:        map[string]string{"component": "connector-hpx-rbr1", "zone": "zcnr06", "peer": "RETAIL_MA"},
		},
		// =====================================================================================================================
		// =====================================================================================================================
		// =====================================================================================================================
