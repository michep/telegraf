...
var (
	re_templates = map[string][]string{
		...
		"advisa": {
			...
			"(?P<hostname>^avi\\w+?)\\.(?P<component>[\\w-]+?\\d+)\\.(?P<midname>[\\w.-]*?(heap|non-heap|-Cache|-Space|-Gen|Metaspace|total))\\.(?P<type>[\\w-]+$) => memory",
			"(?P<hostname>^avi\\w+?)\\.(?P<component>[\\w-]+?\\d+)\\.(?P<midname>[\\w.-]*?(Generation|MarkSweep|Scavenge))\\.(?P<type>[\\w-]+$) => gc",
			"(?P<hostname>^avi\\w+?)\\.(?P<component>[\\w-]+?\\d+)\\.(?P<midname>[\\w.-]+?(Time|Timer|timer|Override|Operations|Merchant|Terminal|DataSource|QueryExecution|OpeningConnection|Duration|Locations))\\.(?P<type>[\\w-]+$) => timer",
			...
			"(?P<hostname>^avi\\w+?)\\.(?P<component>[\\w-]+?\\d+)\\.(?P<midname>[\\w.-]+?IndexService)\\.(?P<type>[\\w-]+$) => index",
			"(?P<hostname>^avi\\w+?)\\.(?P<component>[\\w-]+?\\d+)\\.(?P<midname>[\\w.-]+?)\\.(?P<type>(queueSize|retryQueueSize)$) => queue",
			"(?P<hostname>^avi\\w+?)\\.(?P<component>[\\w-]+?\\d+)\\.(?P<midname>[\\w.-]+?)\\.(?P<type>(notIndexedCount|descriptors)$) => value",
